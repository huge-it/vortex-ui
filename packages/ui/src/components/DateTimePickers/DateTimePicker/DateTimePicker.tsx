...
const MINUTE_OPTIONS = Array.from({ length: 12 }, (_, i) => ({ label: String(i * 5).padStart(2, "0"), value: i * 5 }));
const MERIDIEM_OPTIONS = [{ label: "AM", value: "AM" }, { label: "PM", value: "PM" }];
...
function TimeColumn({ label, items, selected, onSelect }: any) {
  ...
  return (
    <Box sx={{ width: "50px", display: "flex", flexDirection: "column", alignItems: "center" }}>
      <Box sx={{ flex: 1 }}>
        <Box sx={{ textAlign: "center", fontSize: "14px", fontWeight: 400, color: "text.secondary", mb: 1 }}>
          {label}
        </Box>
      </Box>
...
function TimeSelector({ hour, minute, meridiem, onHourChange, onMinuteChange, onMeridiemChange }: any) {
  return (
    <Box sx={{ display: "flex", borderLeft: "1px solid", borderColor: "divider", pt: "8px" }}>
      <TimeColumn label="HH" items={HOUR_OPTIONS} selected={hour} onSelect={onHourChange} />
      <Box sx={{ width: "1px", backgroundColor: "divider", mb: 1.5 }} />
      <TimeColumn label="MM" items={MINUTE_OPTIONS} selected={minute} onSelect={onMinuteChange} />
      <Box sx={{ width: "1px", backgroundColor: "divider", mb: 1.5 }} />
      <TimeColumn items={MERIDIEM_OPTIONS} selected={meridiem} onSelect={onMeridiemChange} />
    </Box>
  );
}
...
export const DateTimePicker = React.forwardRef<HTMLDivElement, DateTimePickerProps>(({
  ...
  const [selectedHour, setSelectedHour] = useState<number | null>(initial ? Number(initial.format("h")) : null);
  const [selectedMinute, setSelectedMinute] = useState<number | null>(initial ? initial.minute() - (initial.minute() % 5) : null);
  const [selectedMeridiem, setSelectedMeridiem] = useState<string | null>(initial ? initial.format("A") : null);
  ...
  const commitIfComplete = (date: Dayjs | null, hour: number | null, minute: number | null, meridiem: string | null) => {
